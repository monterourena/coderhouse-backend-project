...
class HttpResponse {
  constructor(res) {
    this.res = res;
    this.ok = this.customResponse(httpCodes.OK, httpMessages.OK);
    this.created = this.customResponse(httpCodes.CREATED, httpMessages.CREATED);
    this.badRequest = this.customResponse(
      httpCodes.BAD_REQUEST,
      httpMessages.BAD_REQUEST
    );
    this.forbidden = this.customResponse(
      httpCodes.FORBIDDEN,
      httpMessages.FORBIDDEN
    );
    this.notFound = this.customResponse(
      httpCodes.NOT_FOUND,
      httpMessages.NOT_FOUND
    );
    this.internalServerError = this.customResponse(
      httpCodes.INTERNAL_SERVER_ERROR,
      httpMessages.INTERNAL_SERVER_ERROR
    );
  }

  customResponse(defaultCode, defaultMessage) {
    return function ({ message, data, ...params } = {}) {
      const responseMessage = message || defaultMessage;
      return this.res
        .status(defaultCode)
        .send({ meta: { message:responseMessage, ...params }, data });
    };
  }
}
