...
const httpMessages = {
  ...
  NON_AUTHORITATIVE_INFORMATION:
    "The response contains non-authoritative information.",
  NO_CONTENT:
  ...
  PARTIAL_CONTENT:
    "The response contains only a partial representation of the requested resource.",
  MULTI_STATUS:
    "The response provides status for multiple independent operations.",
  ALREADY_REPORTED:
    "The requested resource is already reported and processed in the background.",
  IM_USED:
  ...
  SEE_OTHER:
    "The response is a reference to another URL that should be followed.",
  NOT_MODIFIED:
    "The requested resource has not been modified since the last request.",
  USE_PROXY:
    "The request should be accessed through a proxy specified in the Location header.",
  TEMPORARY_REDIRECT:
    "The request should be redirected to the requested temporary URL.",
  PERMANENT_REDIRECT:
    "The request should be redirected to the requested permanent URL.",
  ...
  METHOD_NOT_ALLOWED:
    "The requested resource does not support the provided HTTP method.",
  NOT_ACCEPTABLE:
  ...
  UPGRADE_REQUIRED:
    "The client should switch to a different protocol to fulfill the request.",
  ...
  VARIANT_ALSO_NEGOTIATES:
    "The server has detected an internal configuration error.",
};

class Response {
  // Success
  ok = (res, { message = httpMessages.OK, data = null } = {}) => {
    return res.status(httpCodes.OK).send({ meta: { message }, data });
  };

  created = (res, { message = httpMessages.CREATED, data = null } = {}) => {
    return res.status(httpCodes.CREATED).send({ meta: { message }, data });
  };
  ...
  badRequest = (
    res,
    { message = httpMessages.BAD_REQUEST, data = null } = {}
  ) => {
    return res.status(httpCodes.BAD_REQUEST).send({ meta: { message }, data });
  };

  forbidden = (res, { message = httpMessages.FORBIDDEN, data = null } = {}) => {
    return res.status(httpCodes.FORBIDDEN).send({ meta: { message }, data });
  };

  notFound = (res, { message = httpMessages.NOT_FOUND, data = null } = {}) => {
    return res.status(httpCodes.NOT_FOUND).send({ meta: { message }, data });
  };

  // Server Errors
  internalServerError = (
    res,
    { message = httpMessages.INTERNAL_SERVER_ERROR, data = null } = {}
  ) => {
    return res
      .status(httpCodes.INTERNAL_SERVER_ERROR)
      .send({ meta: { message }, data });
  };
}

const instance = new Response();

export { instance as Response };
